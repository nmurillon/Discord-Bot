module.exports = async (client, Discord, reaction, user) => {
    ...
    if (reaction.emoji.name === carpetteEmoji) {
        await reaction.message.guild.members.cache.get(user.id).roles.remove(carpette);
    }
    if (reaction.emoji.name === clAmieEmoji) {
        await reaction.message.guild.members.cache.get(user.id).roles.remove(clAmie);
    }   
}